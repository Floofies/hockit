...
app.use(handleError);
if (hockit.config.sslCert === "" || hockit.config.sslKey === "") {
	const server = app.listen(hockit.config.port);
...
const httpsConf = {
	key: hockit.config.sslKey,
	cert: hockit.config.sslCert
};
if (hockit.config.sslPass !== "") httpsConf.passphrase = hockit.config.sslPass;
const server = https.createServer(httpsConf, app).listen(hockit.config.port);
server.hockit = hockit;
server.app = app;
module.exports = server;
